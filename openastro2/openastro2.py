...
class mainWindow:
	...
	def tableMonthlyTimeline2(self, widget):
		dialog = Gtk.Dialog(_("Select Month"),
                     self.window,
                     0,
                     (Gtk.STOCK_CANCEL, Gtk.ResponseType.REJECT,
		...
		if ret == Gtk.ResponseType.ACCEPT:
			...
			self.tMTentry['M']=self.tMTentry['M'].get_text()
			dialog.destroy()
			self.tableMonthlyTimelineShow2()
		else:
			dialog.destroy()
		return
	...
	def tableMonthlyTimelineShow(self, printing=None):
		...
		return
	def tableMonthlyTimelineShow2(self, printing=None):
		self.tabletype="timeline"
		y = int(self.tMTentry['Y'])
		year = int(self.tMTentry['Y'])
		m = int(self.tMTentry['M'])
		month = int(self.tMTentry['M'])
		tz = datetime.timedelta(seconds=float(openAstro.timezone)*float(3600))
		startdate = datetime.datetime(y,m,1,12) - tz
		q,r = divmod(startdate.month, 12)
		enddate = datetime.datetime(startdate.year+q, r+1, 1,12) + datetime.timedelta(days=365)
#		enddate = datetime.datetime(startdate.year+q, r+1, 1,12)
		delta = enddate - startdate
		orb_max=2
		atgrid={}
		...
		for d in range(delta.days):
			...
			for i in range(len(openAstro.planets)):
				start=openAstro.planets_degree_ut[i]
				#for x in range(i+1):
				for x in range(len(openAstro.planets)):
					if (i==0) & (x==4) & (d==13):
						i=i
					end=tmoddata.planets_degree_ut[x]
					diff=float(openAstro.degreeDiff(start,end))
					...
					if (openAstro.planets[i]['visible'] == 1) & (openAstro.planets[x]['visible'] == 1):
						for z in range(len(openAstro.aspects)):
							...
							if openAstro.aspects[z]['is_major'] != 1:
								continue
							#if (openAstro.aspects[z]['degree'] != 0) & (openAstro.aspects[z]['degree'] != 60) & (openAstro.aspects[z]['degree'] != 120) :
							#	continue
							...
							if	( float(openAstro.aspects[z]['degree']) - orb_before ) <= diff <= ( float(openAstro.aspects[z]['degree']) + orb_after ):
								...
								if orb < 0:
									orb = orb/-1
								#aspect grid dictionary
								s="%02d%02d%02d"%(i,z,x) #i-planet, z-aspect, x-planet+1
								astypes[s]=(i,x,z) #i-planet, x-planet+1, z-aspect
								#astypes_aspect[s]=openAstro.aspects[z]['degree']
								...
								if s not in atgrid:
									...
									atgrid_aspect[s]={}
								atgrid[s][d]=orb
								atgrid_aspect[s][d]=openAstro.aspects[z]['degree']
		#sort
		keys = list(astypes.keys())
		keys.sort()
		pages = int(math.ceil(len(keys)/65.0))
		...
		c = 0
		for m in range(len(keys)):
			i,x,z = astypes[keys[m]] #i-planet, x-planet+1, z-aspect
			c += 1
			pagenum = int(math.ceil(c/65.0))
			pagey = (pagenum - 1) * 200
			y = (c*16) + pagey
			...
			for d in range(delta.days):
				if d in atgrid[keys[m]]:
					orb = atgrid[keys[m]][d]*1#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					#op = .1+(.7-(orb/(orb_max/.7))) #4 is maxorb
					op = .5+(.7-(orb/(orb_max/.7)))*10 #4 is maxorb
					op_str = str(int(op))

					if d not in sumorbplanet:
						sumorbplanet[d]={};
						sumorbplanet_pos[d]={};
						sumorbplanet_neg[d]={};
						sumorbplanetlable[d]={};
					if x not in sumorbplanet[d]:
						sumorbplanet[d][x]=0;
						sumorbplanet_neg[d][x]=0;
						sumorbplanet_pos[d][x]=0;
						sumorbplanetlable[d][x]="";
					if i not in sumorbplanet[d]:
						sumorbplanet[d][i]=0;
						sumorbplanet_neg[d][i]=0;
						sumorbplanet_pos[d][i]=0;
						sumorbplanetlable[d][i]="";
					sumorbplanet[d][x] = sumorbplanet[d][x] + op;
					sumorbplanetlable[d][x]=openAstro.planets[x]['label'];
					sumorbplanet[d][i] = sumorbplanet[d][i] + op;
					sumorbplanetlable[d][i]=openAstro.planets[i]['label'];


					op_sign=''
					if d not in sumorb:
						sumorb[d]=0
					sumorb[d] = sumorb[d]+op
					if d not in sumorb_pos:
						sumorb_pos[d]=0
					if ((atgrid_aspect[keys[m]][d] == 60) | (atgrid_aspect[keys[m]][d] == 120)):
						#if (atgrid_aspect[keys[m]][d] == 0):
						sumorb_pos[d] = sumorb_pos[d]+op
						sumorbplanet_pos[d][i] = sumorbplanet_pos[d][i]+op
						sumorbplanet_pos[d][x] = sumorbplanet_pos[d][x]+op
						op_sign="+"
					if d not in sumorb_neg:
						sumorb_neg[d]=0
					if ((atgrid_aspect[keys[m]][d] == 90) | (atgrid_aspect[keys[m]][d] == 180)):
						sumorb_neg[d] = sumorb_neg[d]+op
						sumorbplanet_neg[d][i] = sumorbplanet_neg[d][i]+op
						sumorbplanet_neg[d][x] = sumorbplanet_neg[d][x]+op
						op_sign="-"
					if (atgrid_aspect[keys[m]][d] == 0):
						#if (atgrid_aspect[keys[m]][d] == 0):
						sumorb_pos[d] = sumorb_pos[d]+op/2
						sumorbplanet_pos[d][i] = sumorbplanet_pos[d][i]+op/2
						sumorbplanet_pos[d][x] = sumorbplanet_pos[d][x]+op/2
						sumorb_neg[d] = sumorb_neg[d]+op/2
						sumorbplanet_neg[d][i] = sumorbplanet_neg[d][i]+op/2
						sumorbplanet_neg[d][x] = sumorbplanet_neg[d][x]+op/2
						op_sign="+-"
					if d not in sumoasp_txt:
						sumoasp_txt[d]=''
						sumoasp_all_txt[d]=''
						asp_txt[d]={}
						asp_txt_op[d]={}
						asp_txt_op_sign[d]={}
					sumoasp_txt[d] = sumoasp_txt[d] + openAstro.planets[x]['label'] + "" + str(openAstro.aspects[z]['degree']) + "" + openAstro.planets[i]['label'] + " " + op_sign + op_str + "" + "\n"
					if m not in asp_txt[d]:
						asp_txt[d][m]=''
						asp_txt_op[d][m]=0
						asp_txt_op_sign[d][m]=""
					asp_txt[d][m] = openAstro.planets[x]['label'] + "" + str(openAstro.aspects[z]['degree']) + "" + openAstro.planets[i]['label']
					asp_txt_op[d][m] = op
					asp_txt_op_sign[d][m] = op_sign
					...
					#	op = 10#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					strop = str(float(orb))
					strop2 = str(float(sumorb_pos[d]*1))
...
#					dx[skipdays[s]+1]-5,y-1,dx[skipdays[s+1]]+15,y-1,openAstro.colors['paper_0'])
		...
		for d in range(delta.days):
			strop2 = str(float(sumorb_pos[d]*1))
			out += '<text x="%s" y="%s" style="fill: %s; font-size: 10">%s</text>\n'%(
				dx[d],y+9,openAstro.colors['paper_0'],strop2[:3])
			f.write(strop2[:3] + ';')
			# event = Event()
			# event.add('summary', '+' + strop2[:3])
			# event.add('dtstart', datetime.datetime(year, month, d+1, 0, 0, 0, tzinfo=pytz.utc))
			# event.add('dtend',   datetime.datetime(year, month, d+1, 0, 1, 0, tzinfo=pytz.utc))
			# event.add('dtstamp', datetime.datetime(year, month, d+1, 0, 0, 0, tzinfo=pytz.utc))
...
#			cal.add_component(event)
		...
		for d in range(delta.days):
			strop2 = str(float(sumorb_neg[d]*1))
			out += '<text x="%s" y="%s" style="fill: %s; font-size: 10">%s</text>\n'%(
				dx[d],y+9,openAstro.colors['paper_0'],strop2[:3])
			f.write(strop2[:3] +  ';')
			# event = Event()
			# event.add('summary', '-' + strop2[:3])
			# event.add('dtstart', datetime.datetime(year, month, d+1, 0, 0, 0, tzinfo=pytz.utc))
			# event.add('dtend',   datetime.datetime(year, month, d+1, 23, 59, 59, tzinfo=pytz.utc))
			# event.add('dtstamp', datetime.datetime(year, month, d+1, 0, 0, 0, tzinfo=pytz.utc))
			# # Adding events to calendar
#			cal.add_component(event)
		...
		for d in range(delta.days):
			if sumorb_pos[d] >= sumorb_neg[d]:
				tcolor="💚"
			else:
#				tcolor="🔴"
				tcolor="⚠"
				
			sumoasp_all_txt[d] = tcolor + ' ' + str(int(sumorb_pos[d]+sumorb_neg[d])) + " (+" + str(int(sumorb_pos[d]*1)) + " -" + str(int(sumorb_neg[d]*1)) + ")"
			sumoasp_txt[d] =  sumoasp_txt[d] + "\n"
			f.write(sumoasp_txt[d] +  ';')
			event = Event()
#			event.add('summary', sumoasp_txt[d])
			event.add('dtstart', datetime.date(year, month, 1) + datetime.timedelta(days=d))
			event.add('dtend',   datetime.date(year, month, 1) + datetime.timedelta(days=d))
			event.add('dtstamp', datetime.datetime(year, month, 1, 0, 0, 0, tzinfo=pytz.utc) + datetime.timedelta(days=d))
			descrip=''
			# for i in range(len(openAstro.planets)):
			# 	if i in sumorbplanet[d]:
			# 		descrip = descrip + sumorbplanetlable[d][i] + "" + str(sumorbplanet[d][i])[:3] + "; "

			a=sumorbplanet[d]
			b=sumorbplanetlable[d]
			sorted_tuple = sorted(a.items(), key=lambda x: x[1], reverse=True)
			a1 = dict(sorted_tuple)
#			a1, b1 = zip(*sorted(zip(a, b)))
#			indices = sorted(range(len(a)), key=lambda a: a[ii])
			a1_keys = a1.keys()
			summary_txt = ""
			iii=0
			for i in a1_keys:
				ii=i
				if ii in sumorbplanet[d]:
					if sumorbplanet_pos[d][ii] >= sumorbplanet_neg[d][ii]:
						tcolor="💚"
					else:
						tcolor="🔴"
					descrip = descrip + tcolor + ' ' + sumorbplanetlable[d][ii] + " " + str(int(sumorbplanet[d][ii])) + "(+" + str(int(sumorbplanet_pos[d][ii])) + ",-" + str(int(sumorbplanet_neg[d][ii])) + ")" +"\n"
					if iii<3:
						summary_txt = summary_txt + sumorbplanetlable[d][ii] + " "
					iii = iii + 1
...
#			indices = sorted(range(len(a)), key=lambda a: a[ii])
			...
			for i in a1_keys:
				ii=i
				if ii in asp_txt_op[d]:
					if asp_txt_op_sign[d][ii] == "+":
						tcolor="💚"
					else:
						tcolor="🔴"
					descrip = descrip + tcolor+' ' + asp_txt[d][ii] + " " + asp_txt_op_sign[d][ii] + str(int(asp_txt_op[d][ii]))  +"\n"
					#descrip = descrip + asp_txt[d][ii] +"\n"
					if iii<3:
						summary_asp_txt = summary_asp_txt + asp_txt[d][ii] + " "
					iii = iii + 1


			#event.add('description', sumoasp_all_txt[d] + "\n" + descrip + " \n" + sumoasp_txt[d])
			event.add('description', sumoasp_all_txt[d] + "\n\n" + descrip)
			event.add('summary', sumoasp_all_txt[d] + " " + summary_txt + " " + summary_asp_txt)
...
#		f = open(os.path.join(directory, 'example.ics'), 'wb')
	...
	def aboutInfo(self, widget):
		...
		about_text = _('OpenAstro.org - Open Source Astrology 2.000')+'\n\n'+_('Version')+' '+cfg.version+'\n'+_('Author')+': Pelle van der Scheer'
	...
	def settingsAspects(self, widget):
		...
		for i in range(len(openAstro.aspects)):
			...
			if openAstro.aspects[i]['visible'] == 1:
			...
			if openAstro.aspects[i]['visible_grid'] == 1:
	...
	def settingsPlanets(self, obj):
		...
		for i in range(len(openAstro.planets)):
			...
			data[-1]['visible'] = Gtk.CheckButton()
			if openAstro.planets[i]['visible'] == 1:
				data[-1]['visible'].set_active(True)
			...
			if openAstro.planets[i]['visible_aspect_line'] == 1:
			...
			if openAstro.planets[i]['visible_aspect_grid'] == 1:
